...
public class CustomBuild : IPreprocessBuildWithReport, IPostprocessBuildWithReport
{
    ...
    static string Rename(string path, string newName)
    {
        ...
        UnityEngine.Debug.Log("Name là " + name);
        string newPath = path.Replace(name, newName);
        UnityEngine.Debug.Log("New output path là " + newPath);
        return newPath;
    }
    ...
    public static void BuildGame()
    {
        ...
        {
            convert2Apk = EditorUtility.DisplayDialog("Dâng tấu chương", "Tâu bệ hạ , người có muốn convert luôn sang file apk không ạ ?", "Chuẩn tấu", "Đéo muốn");
        }
        addVersionToFinalFile = EditorUtility.DisplayDialog("Dâng tấu chương", "Bẩm Hoàng Thượng , người có muốn thêm version name_code vào cuối file không ?", "Chuẩn tấu", "Bỏ đi bạn ei");
#endif
    ...
    static void CheckAndRemoveReporter()
    {
        ...
        if (reporter != null)
        {
            bool removeReporter = EditorUtility.DisplayDialog("Dâng tấu chương", "Bẩm Hoàng Thượng , chúng thần phát hiện REPORTER ", "Trảm", "Ân Xá");
            if (removeReporter)
            {
